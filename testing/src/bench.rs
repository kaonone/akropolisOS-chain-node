	...
	pub fn generate_block(&mut self, block_type: BlockType) -> Block {
		...
		for _ in 0..block_type.transactions() {
			...
			let signed = self.keyring.sign(
				CheckedExtrinsic {
					signed: Some((sender, signed_extra(0, akropolisos_runtime::ExistentialDeposit::get() + 1))),
					function: Call::Balances(
						BalancesCall::transfer(
							pallet_indices::address::Address::Id(receiver),
							match block_type {
								BlockType::RandomTransfers(_) => akropolisos_runtime::ExistentialDeposit::get() + 1,
								BlockType::RandomTransfersReaping(_) => 100*DOLLARS - akropolisos_runtime::ExistentialDeposit::get() - 1,
							}
						)
					),
				},
...
impl BenchKeyring {
	...
	pub fn generate_genesis(&self) -> akropolisos_runtime::GenesisConfig {
		crate::genesis::config_endowed(
			false,
			Some(akropolisos_runtime::WASM_BINARY),
