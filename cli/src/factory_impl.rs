...
use akropolisos_runtime::{Signature, 
...
type Number = <<akropolisos_runtime::Block as BlockT>::Header as HeaderT>::Number;

impl<Number> FactoryState<Number> {
	fn build_extra(index: akropolisos_runtime::Index, phase: u64) -> akropolisos_runtime::SignedExtra {
		(
...
impl RuntimeAdapter for FactoryState<Number> {
	type AccountId = akropolisos_runtime::AccountId;
	type Balance = akropolisos_runtime::Balance;
	type Block = akropolisos_runtime::Block;
	type Phase = sp_runtime::generic::Phase;
	type Secret = sr25519::Pair;
	type Index = akropolisos_runtime::Index;
