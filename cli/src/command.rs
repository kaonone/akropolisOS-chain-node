	...
	match opt.subcommand {
		...
		Some(Subcommand::Inspect(cmd)) => {
			...
			let client = sc_service::new_full_client::<
				akropolisos_runtime::Block, akropolisos_runtime::RuntimeApi, node_executor::Executor,
			>(&config)?;
			let inspect = node_inspect::Inspector::<akropolisos_runtime::Block>::new(client);
		...
		Some(Subcommand::Benchmark(cmd)) => {
			...
			cmd.run::<akropolisos_runtime::Block, node_executor::Executor>(config)
		},
