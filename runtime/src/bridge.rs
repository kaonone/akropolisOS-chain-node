...
use crate::token;
use crate::types::*;
use num_traits::ops::checked::{CheckedAdd, CheckedMul, CheckedDiv, CheckedSub};
use num_traits::bounds::Bounded;
use parity_codec::Encode;
use primitives::H160;
use runtime_primitives::traits::{As, Hash};
use support::{
    decl_event, decl_module, decl_storage, dispatch::Result, fail, ensure, StorageMap, StorageValue,
};
...
decl_storage! {
    trait Store for Module<T: Trait> as Bridge {
        ...
        LimitMessages get(limit_messages): map T::Hash  => LimitMessage<T::Hash, T::Balance>;
        ...
        // open transactions
        CurrentPendingBurn get(pending_burn_count): T::Balance;
        CurrentPendingMint get(pending_mint_count): T::Balance;
        ...
        TransferMessages get(messages): map T::Hash  => TransferMessage<T::AccountId, T::Hash, T::Balance>;
        ...
        DailyLimits get(daily_limits_by_account): map (TokenId, T::AccountId)  => T::Balance;
        DailyBlocked get(daily_blocked): map (TokenId, T::Moment)  => Vec<T::AccountId>;
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn set_transfer(origin, to: H160, token_id: TokenId, #[compact] amount: T::Balance)-> Result
        {
        ...
        pub fn multi_signed_mint(origin, message_id: T::Hash, from: H160, to: T::AccountId, token_id: TokenId, #[compact] amount: T::Balance)-> Result {
        ...
        pub fn update_limits(origin, max_tx_value: T::Balance, day_max_limit: T::Balance, day_max_limit_for_one_address: T::Balance, max_pending_tx_limit: T::Balance,min_tx_value: T::Balance)-> Result {
            let validator = ensure_signed(origin)?;
            Self::check_validator(validator.clone())?;
            let limits = Limits{
                max_tx_value,
        ...
        pub fn approve_transfer(origin, message_id: T::Hash) -> Result {
            let validator = ensure_signed(origin)?;
            ensure!(Self::bridge_is_operational(), "Bridge is not operational");
            Self::check_validator(validator.clone())?;
        ...
        pub fn update_validator_list(origin, message_id: T::Hash, quorum: u64, new_validator_list: Vec<T::AccountId>) -> Result {
            let validator = ensure_signed(origin)?;
            Self::check_validator(validator.clone())?;
        ...
        pub fn pause_bridge(origin) -> Result {
            let validator = ensure_signed(origin)?;
            Self::check_validator(validator.clone())?;
        ...
        pub fn resume_bridge(origin) -> Result {
            let validator = ensure_signed(origin)?;
            Self::check_validator(validator.clone())?;
        ...
        pub fn confirm_transfer(origin, message_id: T::Hash) -> Result {
            let validator = ensure_signed(origin)?;
            ensure!(Self::bridge_is_operational(), "Bridge is not operational");
            Self::check_validator(validator.clone())?;
        ...
        pub fn cancel_transfer(origin, message_id: T::Hash) -> Result {
            let validator = ensure_signed(origin)?;
            Self::check_validator(validator.clone())?;
        ...
        fn on_finalize() {
            ...
            for t in tokens {
                if <DailyBlocked<T>>::exists((t.id, yesterday.clone())) && !is_first_day {
                    let blocked_yesterday = <DailyBlocked<T>>::get((t.id, yesterday.clone()));
                blocked_yesterday.iter().for_each(|a| <DailyLimits<T>>::remove((t.id, a.clone())));
                blocked_yesterday.iter().for_each(|a|{
                    ...
                    <DailyBlocked<T>>::remove((t.id, yesterday.clone()));
            }
        }
    }
}
...
impl<T: Trait> Module<T> {
    ...
    fn check_daily_account_volume(
        ...
        let cur_pending = <DailyLimits<T>>::get((token_id, account.clone()));
        ...
        let user_blocked = <DailyBlocked<T>>::get((token_id, today.clone()))
            .iter()
            .any(|a| *a == account);

        if !can_burn {
            <DailyBlocked<T>>::mutate((token_id, today.clone()), |v| {
    ...
    fn check_daily_holds(message: TransferMessage<T::AccountId, T::Hash, T::Balance>) -> Result {
        ...
        if !day_passed {
            ...
            let allowed_amount = account_balance
                .checked_div(&T::Balance::sa(100))
                .expect("Failed to calculate allowed withdraw amount")
                .checked_mul(&T::Balance::sa(75))
                .expect("Failed to calculate allowed withdraw amount");
