...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn make_investment(origin, proposal_id: u64) -> Result {
...
impl<T :Trait> Module<T> {
    pub fn propose_to_investment(dao_id: DaoId, description: Vec<u8>, days: Days, rate: Rate, value: T::Balance) -> Result {

        Self::deposit_event(RawEvent::ProposeToInvestment(dao_id, description, days, rate, value));
        Ok(())
    }
}
...
mod tests {
    ...
    fn propose_to_investment_should_work() {
		...
		with_externalities(&mut new_test_ext(), || {
            assert_ok!(Marketplace::propose_to_investment(DAO_ID, DAO_DESC.to_vec(), DAYS, RATE, VALUE));
        });
    }
}
